...
**LOReL Sawyer Dataset:** Next is the Language-conditioned Offline Reward Learning (LORel) Sawyer dataset, which includes 50,000 pseudo-expert trajectories collected from a replay buffer of a random reinforcment learning (RL) policy. Also these trajectories are annotated with crows-sourced language instructions.
...
The Evaluation was done using both full robot state observations and partially observed images.