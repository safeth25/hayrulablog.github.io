...
As seen in the Graphic above the skill predictor $$f$$ becomes an input of language instruction and a sequence of observations $$ \tau(l, \{s_t, a_t\}^T_{t = 1})$$. Then it predicts a skill code $$ \widetilde{z} \in \mathcal{R} $$ where $$ \widetilde{z} = f(l, (s_t, s_{t-1}, \ldots))$$.
<!-- $$ \widetilde{z} \in \mathcal{R} $$ where $$ \widetilde{z} = f(l, (s_t, s_{t-1, \ldots}))$$. The Vector Quantization $$q(\cdot)$$ maps to $$z$$ to a discrete learnt representation: $$ z = q(\widetilde{z}) =: arg min_{z^k \in \mathcal{C}} || \widetilde{z} - z^k||_2$$ -->
