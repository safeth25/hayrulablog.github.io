...
Having a better understanding of how IL works, let's see how we can define IL formally. In IL the environment is represented as a Markov Decision Process (MDP). The environment in MDP consists a set of States $$S$$, a set of Actions $$A$$, a transition model $$P(s'|s, a)$$ which represents the probability of that an action a in the state s leads to state s’ and a reward function $$R(s,a)$$ that has to be learned. The agent then acts in this environment based on the policy &pi;. 

Since we focus on language-conditioned skill learning, we augment the MDP with a set of different tasks $\mathcal{T}$
