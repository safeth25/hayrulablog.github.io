...
Vector Quantization (VQ) is then used to map this continuous embedding $$\tilde{z}$$ to a discrete representation: $$ z = q(\widetilde{z}) =:$$ arg min$$_{z^k \in \mathcal{C}} \Vert \widetilde{z} - z^k \Vert_2$$
<!-- As seen in the Graphic above the skill predictor $$f$$ becomes an input of language instruction and a sequence of observations $$ \tau = (l, \{s_t, a_t\}^T_{t = 1})$$. Then it predicts a skill code $$\widetilde{z} \in \mathcal{R}^{\mathcal{D}}$$ where $$\widetilde{z} = f(l, (s_t, s_{t-1}, \ldots))$$. The Vector Quantization $$q(\cdot)$$ maps $$\tilde{z}$$ to a discrete learnt representation: $$ z = q(\widetilde{z}) =:$$ arg min$$_{z^k \in \mathcal{C}} \Vert \widetilde{z} - z^k \Vert_2$$. -->
