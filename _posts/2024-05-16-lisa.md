...
Here, $$ \mathcal{C} = \{z^1, \ldots, z^K\} $$ represents the Codebook containing $$K$$ discrete vectors, each representing a unique skill learned from the data. The goal is to map the continuozs embedding $$\tilde{z}$$ to the nearest vector $$z$$ in the codebook, similar to $$k$$-means clustering.
