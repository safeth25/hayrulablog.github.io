...
**LOReL Sawyer Dataset:** Next is the Language-conditioned Offline Reward Learning (LORel) Sawyer dataset, which includes 50,000 pseudo-expert trajectories collected from a replay buffer of a random reinforcment learning (RL) policy. Furthermore, these trajectories are annotated with crows-sourced language instructions.
