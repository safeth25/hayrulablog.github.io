...
**The Policy Network:** The skill predictor passes the encoded sub-tasks to the policy network, defines as  &phi; $$: \mathcal{S} \times \mathcal{C} \rightarrow \mathcal{A}$$. Based on these the policy predicts what actions the machine should take at each time step and the encoded skill at that time step. The policy also considers the current state of the environment as well as the skill code to make decisions about the next action. 

How LISA Architecture works. Diagramm taken from the original paper: 
Im writing here as test<img src="/hayrulablog.github.io/images/lisa2.png" alt="LISA Example" align="center" width="300"/> 