...
**BabyAI Dataset:** The BabyAI dataset contains 19 levels of increasing difficulty in a 7x7 grid world. The agent has to perform different tasks like moving objects between rooms, opening or closing doors with only partially observed state and a language instruction. Only a small fraction (0.1% to 10%) of the 1 million language conditioned trajectories was utilized for the experiments.  

<div align="center">
<img src="/hayrulablog.github.io/images/babyAIDATA.png" alt="LISA Training Algorithm" align="center" width="400"/> 
</div>
<!-- First up is the BabyAI dataset, which features 19 levels of increasing difficulty in a 7x7 grid world. Agents use a partially observed ego-centric view to perform tasks like moving objects or opening doors, guided by natural language instructions. Only a small fraction (0.1% to 10%) of the 1 million provided expert trajectories was utilized for the experiments. The evaluation involves testing on 100 different instructions for each level, with a focus on unseen environments and instructions. -->

**LOReL Sawyer Dataset:** Test