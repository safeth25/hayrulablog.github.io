...
<!-- The skill predictor is a function defined as: $$ f: L \times \mathcal{S} \rightarrow \mathcal{C}$$, where $$\mathcal{C} = \{z^1, \ldots, z^K\}$$ is a Codebook of $$K$$ quantized skills. -->
...
As seen in the graphic above the skill predictor $$f$$, which is a function defines as: 
<div align="center">
$$ f: L \times \mathcal{S} \rightarrow \mathcal{C}$$
</div>
becomes an input of language instruction and a sequence of observations $$ \tau = (l, \{s_t, a_t\}^T_{t = 1})$$. The predictor generates a continuous skill embedding $$\widetilde{z} \in \mathcal{R}^{\mathcal{D}}$$, where $$\widetilde{z} = f(l, (s_t, s_{t-1}, \ldots))$$.
