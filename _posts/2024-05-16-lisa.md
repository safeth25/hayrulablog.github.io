...
**Inverse Reinforcement Learning (IRL):** The main idea of IRL is to try to learn the reward function based on the expert's demonstrations, instead of directly learing actions like in BC. After getting the reward function, reindorcement learning is used to maximize the reward function (optimal policy).
